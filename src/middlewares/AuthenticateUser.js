...
const AuthenticateUser = async (req, res, next) => {
    ...
    if (customerUser == null) {
        ...
        if (userInfo?._id) {
            ...
            req.role = 'seller'
            res.cookie('accessToken', accessToken).cookie('_id', ((sellerUser._id).toHexString())).cookie('profilePicUrl', sellerUser.profilePicUrl)
            next()
            return
        }
        ...
        req.role = 'seller'
        res.cookie('accessToken', newAccessToken).cookie('_id', ((sellerUser._id).toHexString())).cookie('profilePicUrl', sellerUser.profilePicUrl)
        next()
        return
    }
    ...
    if (userInfo?._id) {
        ...
        req.role = 'customer'
        res.cookie('accessToken', accessToken).cookie('_id', ((customerUser._id).toHexString())).cookie('profilePicUrl', customerUser.profilePicUrl)
        next()
        return
    }
    ...
    req.role = 'customer'
    res.cookie('accessToken', newAccessToken).cookie('_id', ((customerUser._id).toHexString())).cookie('profilePicUrl', customerUser.profilePicUrl)
    next()
    return
}
